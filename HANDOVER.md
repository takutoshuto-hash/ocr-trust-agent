...
| **実手書き 12 枚（9/26、初見の送り主、育つ前）** | 人の確認 45.0%（氏名・住所は決まりで必ず確認） | **0.00%**（自動確定 154 項目中 0 件） | `eval/out/handwriting_eval.csv`。読み取りそのものの正解率 87.9%（280 項目）。空欄 22 件に作り話 0。間違い 29 件の内訳: 都道府県の省略 4（書いたとおり。郵便番号一覧がサンプル 21 件で補完できず）、数字の読み違い 9（7↔2・9・1）、フリガナ 8（空欄で返した 4 を含む）、漢字 6、住所 2 |
