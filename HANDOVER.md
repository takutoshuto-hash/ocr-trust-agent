...
| 10/6〜12 | 講評反映・予備 |
| 10/13 | 提出（期限の2日前）。作業ブランチを main に取り込む → LICENSE 確認 → `MIN_INSTANCES=1 bash scripts/deploy.sh` → /review に確認待ちを数枚残す → 審査向け情報に認証情報を記入 → 提出 |

本人にお願い中: 実手書き 20〜30 枚、現場の言葉 300 字（1日の枚数・繁忙期・担当人数・事故の経験）、動画の事故は「FAX の劣化で郵便番号の 7 が 1 に見える」で台本を作成済み（別の事故にしたければ `scripts/demo_story.py` の DEFECT を変える）、ライセンス（MIT 想定）とプロジェクト名の英語表記、Zenn の提出フォーム／オフィスアワーの手続き確認。
