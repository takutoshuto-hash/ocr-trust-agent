...
class FirestoreStore:
    ...
    def list_pending(self, limit=50):
        q = self._c("forms").where("status", "==", "pending").order_by("created_at", direction=firestore.Query.DESCENDING).limit(limit)
    ...
    def recent_confirmed(self, sender_id, format_id, limit=3):
        q = (self._c("forms").where("status", "==", "confirmed").where("sender_id", "==", sender_id)
             .order_by("created_at", direction=firestore.Query.DESCENDING).limit(limit))
        out = []
        for d in q.stream():
            fd = FormDecision.model_validate(d.to_dict())
            if fd.final:
                out.append((f"sender={fd.sender_id}", fd.final))
        return out
    ...
    def list_audit(self, form_id=None, limit=200):
        ...
        return [AuditEvent.model_validate(d.to_dict()) for d in q.stream()]
